...
def ProtoFromDataFrames(dataframes):
  ...
  Args:
    dataframes: A list of dicts describing tables for each dataset for the
        proto. Each entry contains a 'table' field of the dataframe of the data
  ...
  for dataframe in dataframes:
    ...
    table_entries = {}
    for col in table:
      table_entries[col] = NdarrayToEntry(table[col])
    datasets.append({'entries': table_entries,
                     'size': len(table),
                     'name': dataframe['name']})
  return GetDatasetsProto(datasets)
...
def DtypeToType(dtype):
  ...
  if dtype.char in np.typecodes['AllFloat']:
    return GetFeatureStatsProtoDef().FLOAT
  elif (dtype.char in np.typecodes['AllInteger'] or dtype == np.bool or
        np.issubdtype(dtype, np.datetime64) or
        np.issubdtype(dtype, np.timedelta64)):
    return GetFeatureStatsProtoDef().INT
  else:
    return GetFeatureStatsProtoDef().STRING


def DtypeToNumberConverter(dtype):
  """Converts a Numpy dtype to a converter method if applicable.

    The converter method takes in a numpy array of objects of the provided dtype
    and returns a numpy array of the numbers backing that object for statistical
...
def NdarrayToEntry(x):
  ...
  data_type = DtypeToType(x.dtype)
  converter = DtypeToNumberConverter(x.dtype)
  flattened = x.ravel()
  ...
  flattened = ([x for x in flattened if str(x) != 'nan']
               if data_type == GetFeatureStatsProtoDef().STRING
               else flattened[~np.isnan(flattened)].tolist())
  missing = orig_size - len(flattened)
  return {'vals': flattened,
          'counts': row_counts,
          'missing': missing,
          'type': data_type}


def GetDatasetsProto(datasets, features=None):
  ...
  Args:
    ...
    features: A list of strings that is a whitelist of feature names to create
        feature statistics for. If set to None then all features in the dataset
  ...
  all_datasets = _GetDatasetsProtoDef()()
  ...
  for outer_dataset in datasets:
    for key, value in outer_dataset['entries'].items():
      # If we have a feature whitelist and this feature is not in the whitelist
      ...
      feature_type = value[
          'type'] if 'type' in value else GetFeatureStatsProtoDef().INT
      # Process the found feature for each dataset.
      for j, dataset in enumerate(datasets):
        feat = all_datasets.datasets[j].features.add(type=feature_type,
                                                     name=key)
        value = dataset['entries'].get(key)
        has_data = value is not None and (
            value['vals'].size != 0 if isinstance(value['vals'], np.ndarray)
            else value['vals'])
        commonstats = None
        # For numeric features, calculate numeric statistics.
        if feat.type in (GetFeatureStatsProtoDef().INT,
                         GetFeatureStatsProtoDef().FLOAT):
          ...
          if has_data:
            ...
            # values in order to calculate histogram buckets/counts. The inifite
            # values will be added back to the first and last buckets.
            nums = nums[np.isfinite(nums)]
            counts, buckets = np.histogram(nums)
            hist = featstats.histograms.add()
            hist.type = GetHistogramProtoDef().STANDARD
            ...
            _PopulateQuantilesHistogram(featstats.histograms.add(),
                                        nums.tolist())
        elif feat.type == GetFeatureStatsProtoDef().STRING:
          ...
          if has_data:
            ...
            for val_index, val in enumerate(sorted_vals):
              ...
              bucket = featstats.rank_histogram.buckets.add(low_rank=val_index,
                                                            high_rank=val_index,
                                                            sample_count=val[0],
                                                            label=printable_val)
              if val_index < 2:
        ...
        if has_data:
          ...
          _PopulateQuantilesHistogram(commonstats.num_values_histogram,
                                      value['counts'])
        else:
...
def _PopulateQuantilesHistogram(hist, nums):
  ...
  quantiles_to_get = [
      x * 100 / num_quantile_buckets for x in range(
          num_quantile_buckets + 1)]
  quantiles = np.percentile(nums, quantiles_to_get)
  hist.type = GetHistogramProtoDef().QUANTILES
  quantiles_sample_count = float(len(nums)) / num_quantile_buckets
  for low, high in zip(quantiles, quantiles[1:]):
    hist.buckets.add(
        low_value=low,
        high_value=high,
        sample_count=quantiles_sample_count)


def GetFeatureStatsProtoDef():
  """Returns the proto from the feature statistics namespace.

     Uses the proto global DB to get the feature statistics protos needed by
     the functions in this module. The use of the global DB is needed so that
     this module can be used in colab through an adhoc_import.

  Returns:
    The feature name stats proto.
  """
  return fs.FeatureNameStatistics


def _GetDatasetsProtoDef():
  """Returns the proto from the feature statistics namespace.
  ...
  Returns:
    The datasets proto.
  """
  return fs.DatasetFeatureStatisticsList


def GetHistogramProtoDef():
  """Returns the proto from the feature statistics namespace.
