...
# ==============================================================================
"""Code for generating the feature_statistics proto.
...
def ProtoFromTfRecordFiles(files, max_entries=10000, features=None,
                           is_sequence=False, iterator_options=None):
  """Creates a feature statistics proto from a set of TFRecord files.

  Args:
    files: A list of dicts describing files for each dataset for the proto. Each
        entry contains a 'path' field with the path to the TFRecord file on disk
  ...
  datasets = []
  for entry in files:
    entries, size = _GetTfRecordEntries(entry['path'], max_entries, is_sequence, iterator_options)
    datasets.append({'entries': entries, 'size': size, 'name': entry['name']})
  return gfsg.GetDatasetsProto(datasets, features)
...
def _GetEntries(paths, max_entries, iterator_from_file, is_sequence=False):
  ...
  for filepath in paths:
    ...
    for record in reader:
      if is_sequence:
        ...
        _ParseExample(sequence_example.context.feature,
                      sequence_example.feature_lists.feature_list, entries,
                      index)
      else:
        _ParseExample(tf.train.Example.FromString(record).features.feature, [],
                      entries, index)
      index += 1
...
def _GetTfRecordEntries(path, max_entries, is_sequence, iterator_options):
  ...
  Returns:
    A tuple with two elements:
        ...
        - The number of examples parsed.
  """
  return _GetEntries([path], max_entries, partial(tf.python_io.tf_record_iterator, options=iterator_options),
                     is_sequence)
